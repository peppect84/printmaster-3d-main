...
import fs from 'fs';
// Rimuovi questa riga o assicurati che punti al percorso corretto se _stai_ usando .env in locale
// Su Render, le variabili d'ambiente sono caricate direttamente.
// import dotenv from 'dotenv'; // <-- Rimuovi o commenta questa riga se Render è l'unico ambiente di deploy
import cors from 'cors';
import axios from 'axios';

// Se stai deployando su Render, questa riga è superflua e potrebbe causare problemi
// perché .env.local non sarà presente. Le variabili su Render sono già in process.env
// dotenv.config({ path: '../.env.local' }); // <-- Rimuovi o commenta questa riga su Render

const app = express();
// Assicurati che Render stia esponendo la porta corretta. Spesso Render imposta process.env.PORT
const port = process.env.PORT || 10000; // Modificato da 5000 a 10000 come suggerito dai tuoi log

// ... (tutto il codice CORS e le interfacce rimangono invariati) ...
...
const sendEmailHandler: RequestHandler = async (req: FormidableRequest, res: Response) => {
    ...
    try {
        ...
        // ... (parsing dei campi del form invariato) ...
        ...
        // Validazione più robusta per i campi chiave e il token hCaptcha
        ...
        // NON leggere da .env.local su Render, leggi direttamente da process.env
        ...
        // ... (il resto del codice per l'invio email rimane invariato) ...
