...
const sendEmailHandler: RequestHandler = async (req: FormidableRequest, res: Response) => {
  ...
  try {
    ...
    try {
        ...
        const { success, 'error-codes': errorCodes } = hcaptchaVerifyResponse.data as { success: boolean; 'error-codes'?: string[] };
  ...
  } catch (error: unknown) { // Usa 'unknown' invece di 'any' per il tipo di errore
